<template>
  <div class="container">
    ...
    <van-cell class="cell">
      <template #title>
        ...
        <span class="server-name">{{
          flag ? createInfo.service.serviceName : "护理陪护（尊享）"
        }}</span>
      </template>
      <template #default>
    ...
    <van-popup v-model:show="showComponion" position="bottom">
      <van-picker
...
const flag = Number(Info.value.id) === 5;
...
const showTimeConfirm = (item) => {
  //将数组拼接为类似2022-01-01的字符串
  const dateStr = item.selectedValues.join("-");
  currentDate.value = dateStr;
  //starttime是个number，所以要转换成时间戳
